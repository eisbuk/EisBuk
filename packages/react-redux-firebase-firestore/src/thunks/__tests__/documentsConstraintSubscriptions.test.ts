...
describe("Firestore subscriptions", () => {
  describe("test subscriptions with documents constraint", () => {
    test("should subscribe to provided documents", async () => {
    ...
    test("should update listener with provided documents", async () => {
      const { dispatch, getState } = getTestStore();
      const db = await getAuthTestEnv(createTestSlots);
      // mock `getFirestore` to use firestore from RulesTestContext
      jest.spyOn(firestore, "getFirestore").mockImplementation(() => db as any);
    ...
    test("should update listener with 'unsubscribe' function composed of the one in store (if one exists) and the new one(s)", async () => {
      const { dispatch, getState } = getTestStore();
      const db = await getAuthTestEnv(createTestSlots);
      // mock `getFirestore` to use firestore from RulesTestContext
      jest.spyOn(firestore, "getFirestore").mockImplementation(() => db as any);
    ...
    test("should not dispatch any updates if there are no new documents to subscribe to", async () => {
      const { dispatch, getState } = getTestStore();
      const db = await getAuthTestEnv(createTestSlots);
      // mock `getFirestore` to use firestore from RulesTestContext
      jest.spyOn(firestore, "getFirestore").mockImplementation(() => db as any);
