...
describe("Firestore subscriptions", () => {
  describe("test subscriptions with range constraint", () => {
    test("should subscribe to a given range if so provided", async () => {
    ...
    test("should delete entries from local store when deleted from firestore", async () => {
      const { dispatch, getState } = getTestStore();
      const db = await getAuthTestEnv(createTestSlots);
      // mock `getFirestore` to use firestore from RulesTestContext
      jest.spyOn(firestore, "getFirestore").mockImplementation(() => db as any);
    ...
    test("should update listener with subscribed range", async () => {
      const { dispatch, getState } = getTestStore();
      const db = await getAuthTestEnv(createTestSlots);
      // mock `getFirestore` to use firestore from RulesTestContext
      jest.spyOn(firestore, "getFirestore").mockImplementation(() => db as any);
    ...
    test("should update listener with 'unsubscribe' function composed of the one in store and the new one, if the store listener contains range overlap", async () => {
      const { dispatch, getState } = getTestStore();
      const db = await getAuthTestEnv(createTestSlots);
      // mock `getFirestore` to use firestore from RulesTestContext
      jest.spyOn(firestore, "getFirestore").mockImplementation(() => db as any);
    ...
    test("edge case: if the range in store and the new range have no overlap, should unsubscribe and remove the old range from listener", async () => {
      const { dispatch, getState } = getTestStore();
      const db = await getAuthTestEnv(createTestSlots);
      // mock `getFirestore` to use firestore from RulesTestContext
      jest.spyOn(firestore, "getFirestore").mockImplementation(() => db as any);
    ...
    test("edge case: if the new range is wider (in both directions) then the one in store, should unsubscribe from the old and only subscribe to (and update listener with) the new", async () => {
      const { dispatch, getState } = getTestStore();
      const db = await getAuthTestEnv(createTestSlots);
      // mock `getFirestore` to use firestore from RulesTestContext
      jest.spyOn(firestore, "getFirestore").mockImplementation(() => db as any);
    ...
    test("edge case: if the new range is smaller or equal then the one in store, should not dispatch any updates", async () => {
      const { dispatch, getState } = getTestStore();
      const db = await getAuthTestEnv(createTestSlots);
      // mock `getFirestore` to use firestore from RulesTestContext
      jest.spyOn(firestore, "getFirestore").mockImplementation(() => db as any);
